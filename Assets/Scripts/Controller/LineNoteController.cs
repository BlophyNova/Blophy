    ...
    public class LineNoteController : MonoBehaviour
    {
        ...
        private void Update()
        {
            if (decideLineController.ThisLine.OnlineNotesLength > 0)
                Find_Get_Update_PassHit_Return(decideLineController.ThisLine.onlineNotes, ref lastOnlineIndex, ariseOnlineNotes, endTimeAriseOnlineNotes, true);
        ...
        private static void AddNote2NoteList(List<NoteController> arisedNotes, List<NoteController> endTimeArisedNotes, NoteController note)
        {
            int index = Algorithm.BinarySearch(endTimeArisedNotes, m => m.thisNote.EndTime < note.thisNote.EndTime, false);//寻找这个音符按照endTime排序的话，因在插在什么位置
        ...
        private void FindAndReturnNotes(List<NoteController> ariseNotes, List<NoteController> endTimeAriseNotes, bool isOnlineNote)
        {
