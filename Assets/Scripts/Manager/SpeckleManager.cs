...
namespace Manager
{
    public class SpeckleManager : MonoBehaviourSingleton<SpeckleManager>
    {
        ...
        private void FindNotes(List<NoteController> needFindNotes, List<NoteController> waitNotes)
        {
            ...
            int indexStartJudgeNeedJudgeNote = Algorithm.BinarySearch(needFindNotes, m => m.thisNote.hitTime < currentTime - m.thisNote.HoldTime, false);//获取到当前时间-bad所得到的需要判定的音符的开始界限
            int indexEndJudgeNeedJudgeNote = Algorithm.BinarySearch(needFindNotes, m => m.thisNote.hitTime < currentTime + JudgeManager.bad, false);//获取到当前时间+bad所得到的需要判定的音符的结束界限
    ...
    {//一个斑点类就是一个手指的触摸
        ...
        public Vector2 CurrentPosition => movePath[LoopBackIndex(indexMovePath, -1, lengthMovePath)];//当前位置，返回值为movePath【当前索引（回环索引）】（世界坐标轴）
        ...
        public Speckle(Vector2[] movePath, Vector2 startPoint,
            TouchPhase phase = TouchPhase.Canceled, float beganTime = 0, int thisIndex = 0, int indexMovePath = 0, bool isFullMovePath = false)
        {
        ...
        public float FlickDirection//滑动方向
        {
            get
            {
                ...
                float v = Vector2.SignedAngle(movePath[isFullMovePath ? ^1 : indexMovePath] - movePath[0], Vector2.right);
        ...
        private void ResetIndex(ref int index) => index = 0;//重置一个索引
